...
namespace Seguridad
{
    public partial class SeguridadUsuario : System.Web.UI.Page
    {
        ...
        private void CargarSucursal(int sucursalID)
        {
            ...
            if (sucursalID != 0)
            {
                strQuery = "select * From Consultorio  WHERE EmpresaID   = " + sucursalID + "  ORDER BY EmpresaID";
            }
            using (SqlConnection con = new SqlConnection(strConnString))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    ddlSucursal.DataTextField = "NombreConsultorio";
                    ddlSucursal.DataValueField = "ConsultorioID";
